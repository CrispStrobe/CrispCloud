...
class InternxtCLI {
  ...
  InternxtCLI() : config = ConfigService(
    configPath: p.join(
    ...
    try {
      switch (command) {
        ...
        default:
          ...
          io.stderr.writeln('💡 Use "dart cli.dart help" for available commands');
          io.exit(1);
      }
    } catch (e, stackTrace) {
  ...
  Future<void> handleWebdavStart(ArgResults argResults) async {
    ...
    if (existingPid != null) {
      io.stderr.writeln('❌ WebDAV server may already be running (PID: $existingPid).');
    ...
    try {
      ...
      final fs = InternxtFileSystem(internxtClient: client);
  ...
  Future<void> handleWebdavStop(ArgResults argResults) async {
    ...
    try {
      ...
      if (success) {
        ...
        print('⚠️  Could not terminate process (PID: $pid). It may already be stopped.');
      }
    } catch (e) {
  ...
  Future<void> handleWebdavTest(ArgResults argResults) async {
    ...
    try {
      ...
      if (e is io.SocketException) { // <-- FIX: Use io.SocketException
        print('❌ Connection failed: Server is not running or unreachable at $url');
  ...
  Future<void> handleList(ArgResults argResults) async {
    try {
      ...
      if (resolvedInfo['type'] != 'folder') {
        io.stderr.writeln("❌ Error: Path '$pathToList' is a file, not a folder.");
        io.exit(1);
      }
...
class InternxtClient {
  ...
  void setAuth(Map<String, dynamic> creds) {
    ...
    if (creds.isEmpty) {
        log("⚠️ TRACE: Warning - setAuth called with empty credentials");
    }
  ...
  Future<Uint8List> downloadFileBytes(
    ...
    log('Downloading file to memory (bytes): $remotePath');

    // 1. Resolve path to UUID
    final resolved = await resolvePath(remotePath);
    final fileUuid = resolved['uuid'];

    // 2. Use existing download logic
    // We need to retrieve bridgeUser and userIdForAuth from the hydrated state
    final currentCreds = await config.readCredentials();
    final bUser = currentCreds?['bridgeUser']?.toString() ?? '';
    final uIdAuth = currentCreds?['userId']?.toString() ?? '';

    if (bUser.isEmpty || uIdAuth.isEmpty) {
      throw Exception("Authentication metadata missing. Please re-login.");
    }
  ...
  Future<Map<String, dynamic>> login(String email, String password, {String? tfaCode}) async {
    final cleanEmail = email.toLowerCase().trim();
    log("🚀 TRACE: Starting Login Sequence for $cleanEmail");

    try {
        // STEP 1
        log("💧 Step 1: Requesting Salt (sKey)...");
        ...
        // STEP 3
        log("🚀 Step 3: Authenticating via /auth/login/access...");
        ...
        // STEP 4
        log("💧 Step 4: Hydrating session (getting bucket/root)...");
        ...
        if (hydrationRes.statusCode != 200) {
          throw Exception("Hydration failed: ${hydrationRes.statusCode} - ${hydrationRes.body}");
        }

        final hydrated = json.decode(hydrationRes.body);
        final user = hydrated['user'];
        log("   ✅ Session hydrated. RootFolder: ${user['rootFolderId']}");
...
class InternxtFileSystem extends FilenFileSystem {
  final InternxtClient internxtClient;

  InternxtFileSystem({required this.internxtClient}) 
      : super(client: filen.FilenClient(
          config: filen.ConfigService(configPath: internxtClient.config.configPath)
        ));

  @override
  pkg_file.Directory directory(dynamic path) {
    return InternxtDirectory(client: internxtClient, path: getPath(path), fs: this);
  }

  @override
  pkg_file.File file(dynamic path) {
    return InternxtFile(client: internxtClient, path: getPath(path), fs: this);
  }
}

class InternxtDirectory extends FilenDirectory {
  InternxtDirectory({
    required InternxtClient client, 
    required String path, 
    required InternxtFileSystem fs
  }) : super(
         client: filen.FilenClient(
           config: filen.ConfigService(configPath: client.config.configPath)
         ), 
         path: path, 
         fs: fs
       );
...
class InternxtFile extends FilenFile {
  InternxtFile({
    required InternxtClient client, 
    required String path, 
    required InternxtFileSystem fs
