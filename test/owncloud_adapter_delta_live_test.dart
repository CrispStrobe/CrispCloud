// test/owncloud_adapter_delta_live_test.dart
//
// Live integration tests for the NextcloudClientAdapter delta sync methods
// against the ownCloud 10.15 instance running on localhost:8889.
//
// CrispCloud uses the same NextcloudClientAdapter for both Nextcloud and
// ownCloud since they share WebDAV + OCS APIs. These tests verify delta
// sync works correctly on ownCloud 10.
//
// Run:  flutter test test/owncloud_adapter_delta_live_test.dart --run-skipped
@Tags(['live'])
...
const _ncUrl = 'http://localhost:8889';
const _ncUser = 'admin';
...
void main() {
  ...
  setUp(() async {
    ...
    final configService = NextcloudConfigService(
      configPath: '/tmp/oc_adapter_live_test',
    ...
    tempDir = await Directory.systemTemp.createTemp('oc_adapter_delta_live_');
  });
